...
// TFT_HX8357 (Mega) and TFT_HX8357_Due font definitions
...
// sketch with shorter names in two ways, either with:
//
//    1. FF# where # is a number determined by looking at the list below
//       an example being:
//
//       tft.setFreeFont(FF32); // This is free font Sans Bold Oblique 24pt
//
...
// To print or draw text in the Free Fonts you must first select the font first and use
// GFXFF in drawString:
//    tft.setFreeFont(FM9);
//    tft.drawString("Hello World", x, y, GFXFF);

// When drawing text in GLCD and high rendering speed fonts (FONT2- FONT8)
// you can use for example:
//    tft.drawString("Hello World", x, y, FONT4);
...
#ifdef LOAD_GFXFF // Only include the free fonts if LOAD_GFXFF is defined in User_Setup.h


// >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
// Next we include all possible font files in case they are needed in the sketch
// >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
//
// Reserved for GLCD font  // FF0
//
// Mono spaced fonts
#include <Fonts/GFXFF/FreeMono9pt7b.h>  // FF1 or FM9  <<<< These are the names to use
...
// >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
// Next there are the short names to call up fonts e.g.:
//   tft.setFreeFont(FM9);
// otherwise we would have to use this long line in the sketch:
//   tft.setFreeFont(&FreeMono9pt7b, FreeMono9pt7bBitmaps, FreeMono9pt7bGlyphs);
// >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
//
